...
export async function withAIAdmission(
  ...
  if (!key || !secret || secret.length < 32) throw new Error("ADMISSION_NOT_CONFIGURED");
  ...
  const store: AdmissionStore = {
    async claim(userId, requestId, fingerprint) {
      const result = await admin.rpc("claim_ai_request", { p_user: userId, p_request: requestId, p_fingerprint: fingerprint });
      if (result.error) throw new Error("ADMISSION_UNAVAILABLE");
      return result.data;
    },
    async finish(userId, requestId, token, success, resultId) {
      const result = await admin.rpc("finish_ai_request", { p_user: userId, p_request: requestId, p_token: token, p_success: success, p_result: resultId });
      if (result.error) throw new Error("ADMISSION_UNAVAILABLE");
      return result.data === true;
    },
  };
  return runAdmittedRequest({ ...input, userId: data.user.id }, secret, store, () => operation(data.user.id));
}
