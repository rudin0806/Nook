 ...
 * 질문 하나와 작은 분류값만 만든다.
 */
export const REFLECT_PROMPT_VERSION = "reflect-v5.7-2026-09-22";
...
export const REFLECT_MODE_PROMPTS: Record<ReflectMode, string> = {
  ...
  CORRECTION:
    "사용자가 닫은 전제를 다시 넣거나 정정 이유를 캐지 않는다. 정정 발화에 없는 경쟁 일정·선택지·이유를 새로 만들지 않는다. 정정한 현실 조건의 가능 경계나 중심 결정에 미치는 무게처럼 아직 답하지 않은 한 가지를 묻는다. 정정한 조건을 `~할 수 있어야 ~할 수 있나요?`라는 가능 여부로 되묻거나 `얼마나까지`처럼 조사를 겹치지 않고 범위를 직접 묻는다(예: `매주 시간을 낼 수 있는지가 문제야` → `매주 어느 정도까지 시간을 낼 수 있어요?`). scope는 CENTER, move는 RECOVERY다.",
  CONFUSED:
