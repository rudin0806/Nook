...
**\`분명해진 것\`은 사용자가 정한 것을 모으는 칸이다. 사용자가 말한 것을 모으는 칸이
아니다.** 운영에서 이 구분이 무너져 목록이 전사가 됐다 — 한 턴이 한두 항목을 만들고,
어미만 \`~함\`·\`~느낌\`으로 바뀌었다.

  U:  회사에서 업무에 대한 지식을 배우고 싶어
  X   회사에서 업무에 대한 지식을 배우고 싶음      ← 어미만 바꾼 전사

들어가는 것은 셋뿐이다.

  1. 사용자가 세운 **기준**        "회사를 잘 다닌다고 느끼려면 성취가 있어야 함"
  2. 사용자가 **배제한 것**        "회사 자체가 싫은 것은 아님"
  3. 결정에 쓰이는 **확정된 사실**  "사수가 없음" / "보증금은 모아둔 게 있음"

셋 다 공통점이 있다. **읽으면 중심 질문의 답 후보가 줄어든다.**

들어가지 않는 것.

  - 아직 정해지지 않은 것
    \`고민 중임\`, \`확신하지 못함\`, \`모르겠음\`, \`우려함\`, \`~하려 함\`
    → 망설임은 고민 그 자체다. 적어도 아무것도 좁혀지지 않는다.
  - 사용자가 말했다는 사실 자체
    \`~다고 함\`, \`~라고 말함\`
    → 무엇이 분명해졌는지가 아니라 무엇을 말했는지를 적고 있다.
  - 중심 질문을 되풀이한 것
    중심 질문이 \`도마뱀을 키울까?\`인데 \`도마뱀을 키우고 싶지만 고민 중임\`
    → 질문을 다시 쓴 것이지 답이 좁혀진 것이 아니다.

**턴마다 하나씩 만들지 않는다.** 대부분의 턴은 아무것도 만들지 않는다. 한 턴이
꼬박꼬박 한 항목을 낳고 있으면 그것은 전사이지 정리가 아니다. 빈 배열이 정상이다.

사용자가 여러 기준을 한 번에 말했을 때만 한 턴에서 둘 이상을 만든다.

  U:  성취, 같이 일하는 사람들이 열정적으로 일하는 것
  O   "…느끼려면 성취가 있어야 함" + "…열정적으로 일해야 함"   ← 기준 둘
...
**AI가 사실을 캐물어 받아낸 답이 특히 위험하다.** 사용자가 답하긴 했지만 중심 질문을
한 칸도 움직이지 않는다.
