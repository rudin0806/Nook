...
CLOSE는 사용자가 현재 질문의 yes/no 결론을 냈다는 뜻이 아니고, 세션을 강제로 끝내는 명령도 아니다.
마지막 사용자 응답에서 더 정리된 재료가 없다는 상태 판정이다. Judge가 CLOSE를 내면 제품이 "여기까지 정리하기"와
"조금 더 보기"를 제안하고 실제 종료 여부는 사용자가 고른다. 따라서 결론이 미정이라는 이유만으로
REFLECT를 계속하지 않는다.
...
여기서 "새 정보 없음"은 입력 창 전체에 새 정보가 한 번도 없었다는 뜻이 아니다. **마지막 사용자
응답이 그 직전까지 나온 발화와 current_clarifications에 새 재료를 더했는지**를 본다. 창 앞부분에서
새 정보가 나와 이미 한두 번 확인했더라도, 마지막 응답이 기존 사실·이유를 반복하기만 하면 CLOSE할 수 있다.
...
이미 current_clarifications에 있는 사실을 현재 창에서 그대로 반복하고, AI가 제안한 다른 가능성에는
"모르겠어"처럼 새 재료가 없으며, 마지막까지 같은 이유만 되풀이하면 CLOSE다. 기존의 완화형 표현이
단정형 반복으로 선명해진 것은 clarification confidence를 갱신할 수 있지만 새 관점이나 SHIFT는 아니다.

이 조건이 맞으면 **반드시 CLOSE**다. 사용자가 계속할지 그만둘지 아직 선택하지 않았어도 같다.

단, 반복된 내용이 AI가 먼저 만든 원인·성향·해석어이고 사용자는 새 사실 없이 동의만 한 경우는
사용자 생각이 수렴한 것이 아니다. 이 반복은 CLOSE의 보조 조건으로 세지 않고 REFLECT/LOW로 두며,
AI가 만든 해석어도 clarification에 저장하지 않는다.

  기존: 가면 또 하긴 함 / 재미가 없는 건 아님
  현재: 다른 데 쓸지는 모르겠어 / 가면 하긴 해 / 그래도 시간이 아까워
  → 새 재료 없이 기존 구분과 같은 이유가 반복됨 → CLOSE
