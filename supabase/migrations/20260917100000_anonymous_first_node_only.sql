-- 비로그인 사용자는 첫 질문(노드 1)까지 도달해 그 안에서 대화할 수 있고, 두 번째
-- 질문으로 넘어가는 지점에서 계정 연결을 요구한다. 제안된 질문은 이미 화면에 있어
-- 사용자는 무엇을 위해 로그인하는지 보고 결정한다.
--
-- The whole function is restated because a body cannot be patched in place; the
-- only change is the declaration of v_anonymous and the guard in the approve
-- phase, which is where a second question node is created.
