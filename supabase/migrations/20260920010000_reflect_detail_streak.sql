-- 되묻기가 세부로 내려간 횟수를 DB가 센다.
--
-- Prompt D 4.6은 "세부로 내려가는 질문은 연달아 두 번까지"라고 적어 두었지만, 그
-- 규칙은 모델이 자기가 앞서 무엇을 물었는지 매 턴 다시 헤아려야 지킬 수 있는
-- 것이었다. 운영에서 지켜지지 않았다: 중심 질문 "회사를 잘 다닐 수 있을까"에서
-- 성취 → 배울 점 → 책임감 있는 업무로 세 번 내려가며 질문이 멀어졌다.
--
-- 그래서 모델은 방금 만든 질문에 CENTER/DETAIL 라벨만 붙이고, 연속 횟수를 세는 일과
-- 돌아오라고 시키는 일은 코드가 맡는다. Safety classifier가 label만 내고 behavior는
-- 결정론적 mapping이 정하는 것과 같은 구조다.
--
-- 노드가 바뀌면 중심 질문도 바뀌므로 approve/reject/continue에서 0으로 돌린다.
alter table public.conversation_runtime
 add column if not exists detail_streak smallint not null default 0
 check (detail_streak between 0 and 9);

-- The whole function is restated because a body cannot be patched in place; the
-- only change is detail_streak bookkeeping in the output/approve/reject/continue
-- phases. A payload without 'scope' resets the streak, so a deploy in progress
-- never enforces a return on a question it never labelled.
...
begin
 ...
 if p_phase='input' then
  ...
  if kind='REFLECT' then
   ...
   update public.conversation_runtime set last_question_type=p_payload->>'type',carryover=p_payload->'carryover',
    detail_streak=case when p_payload->>'scope'='DETAIL' then least(st.detail_streak+1,9) else 0 end where session_id=p_session;
  ...
  -- A visitor without an account may reach the first question and talk within
  -- it, but moving to a second one is where the account is asked for.
  select coalesce(is_anonymous,false) into v_anonymous from auth.users where id=p_user;
  if v_anonymous then raise exception 'IDENTITY_LINK_REQUIRED'; end if;
  ...
  update public.conversation_runtime set pending=null,carryover='[]',last_question_type=null,detail_streak=0,
  ...
  update public.conversation_runtime set pending=null,carryover='[]',mode='READY',detail_streak=0 where session_id=p_session;
  ...
  update public.conversation_runtime set mode='READY',pending=null,carryover='[]',last_question_type=null,detail_streak=0 where session_id=p_session;
