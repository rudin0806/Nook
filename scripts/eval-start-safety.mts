...
import { safetyInputSchema, safetyOutputSchema } from "../src/schemas/safety.ts";
...
function contactNumbers(contact: unknown): string[] {
  ...
  if (typeof contact === "string") return (contact.match(/\d[\d-]*/g) ?? []).sort();
...
export function validateFixtures(safetyRaw: unknown[], startRaw: unknown[]) {
  ...
  for (const row of safety) {
    const mapped = mapSafety({ label: row.expected.label, category: row.expected.category });
    if (
      mapped.behavior !== row.expected.behavior ||
      JSON.stringify(contactNumbers(mapped.contact)) !== JSON.stringify(contactNumbers(row.expected.contact))
...
export async function evaluate(
  ...
  const report = {
    ...
    safety: {
      evaluated: 0, labelCorrect: 0, categoryCorrect: 0,
      stopToContinue: 0, continueToStop: 0, continueToHandoff: 0,
      handoffToContinue: 0, behaviorMismatch: 0, mappingMismatch: 0,
      cases: [] as { id: string; expectedLabel: string; actualLabel: string; expectedCategory: string; actualCategory: string; expectedBehavior: string; actualBehavior: string }[],
    },
    start: {
      evaluated: 0, labelCorrect: 0, forbiddenKeywordCases: 0,
      cases: [] as { id: string; expectedLabel: string; actualLabel: string; focusRequired: boolean; forbiddenKeywordCount: number }[],
    },
    error: null as null | { id: string; stage: string; code: string },
    notEvaluated: ["Node 0 generation", "semantic wording review", "new boundary fixtures", "OAuth/API/DB end-to-end"],
  };
  let id = "";
  let stage = "SAFETY";
  try {
    for (const row of fixtures.safety) {
      ...
      report.safety.categoryCorrect += Number(actual.category === expected.category);
      report.safety.stopToContinue += Number(expected.behavior === "STOP" && actual.behavior === "CONTINUE");
      report.safety.continueToStop += Number(expected.behavior === "CONTINUE" && actual.behavior === "STOP");
      report.safety.continueToHandoff += Number(expected.behavior === "CONTINUE" && actual.behavior === "HANDOFF");
      report.safety.handoffToContinue += Number(expected.behavior === "HANDOFF" && actual.behavior === "CONTINUE");
      report.safety.behaviorMismatch += Number(actual.behavior !== expected.behavior);
      report.safety.cases.push({
        id, expectedLabel: expected.label, actualLabel: actual.label,
        expectedCategory: expected.category, actualCategory: actual.category,
    ...
    for (const row of fixtures.start) {
      id = row.id;
      const actual = await executeStartClassification(row.input, OPTIONS, transports.classify);
      const text = [actual.info_guidance, actual.focus_question].filter(Boolean).join("\n");
      const forbiddenKeywordCount = row.expected.forbidden_keywords.filter((keyword) => text.includes(keyword)).length;
      ...
      report.start.cases.push({
        id, expectedLabel: row.expected.label, actualLabel: actual.label,
        focusRequired: actual.focus_required, forbiddenKeywordCount,
      });
    }
    report.complete = true;
  } catch (error) {
    const code = error instanceof Error && /^(SAFETY|START|MODERATION)_(INPUT_INVALID|OUTPUT_INVALID|PROVIDER_FAILED|RESPONSE_INCOMPLETE|MAPPING_INVALID)$/.test(error.message)
...
async function main() {
  ...
  const sources = ["eval/safety.jsonl", "eval/start.jsonl", "eval/safety_mapping.json", "src/prompts/prompt-safety.ts", "src/prompts/prompt-start.ts"];
  const hashes = Object.fromEntries(sources.map((path) => [
    path, createHash("sha256").update(readFileSync(path)).digest("hex"),
  ]));
  const readRows = (path: string): unknown[] => readFileSync(path, "utf8").trim().split("\n").map((line) => JSON.parse(line));
  ...
  const usage = { modelCalls: 0, moderationCalls: 0, inputTokens: 0, outputTokens: 0 };
  ...
  console.log(JSON.stringify({ commit: process.env.GITHUB_SHA ?? null, hashes, usage, ...report }, null, 2));
  if (!passed(report)) process.exitCode = 1;
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { console.error("START_SAFETY_EVAL_SETUP_FAILED"); process.exitCode = 1; });
}
