...
import { evaluate, passed, validateFixtures } from "../scripts/eval-start-safety.mts";
...
const moderation = {
  results: [{
    flagged: false,
    categories: Object.fromEntries(MODERATION_CATEGORIES.map((key) => [key, false])),
  }],
};

test("rejects duplicate fixtures and derived mapping drift before calls", () => {
  assert.throws(() => validateFixtures([safety[0], ...safety.slice(0, 14)], start));
  const changed = structuredClone(safety) as { expected: { contact: string } }[];
  changed[0].expected.contact = "999";
  assert.throws(() => validateFixtures(changed, start), /FIXTURE_MAPPING_INVALID/);
});
...
test("scores misrouting separately and never includes raw text in report", async () => {
  ...
  const report = await evaluate(fixtures, {
    ...
    classify: async (request) => {
      ...
      const row = fixtures.start.find((row) => row.input.raw_thought === input.raw_thought)!;
      return {
        status: "completed",
        output_text: JSON.stringify({
          label: row.expected.label,
          focus_required: false, focus_question: null, focus_candidates: [],
          info_guidance: row.expected.label === "NEEDS_INFO" ? "관련 정보를 확인해주세요." : null,
        }),
      };
    },
  });
  ...
  for (const row of fixtures.safety) assert.equal(serialized.includes(row.input.utterance), false);
  for (const row of fixtures.start) assert.equal(serialized.includes(row.input.raw_thought), false);
});

test("provider failure stops both suites without retry or leaking error", async () => {
  let calls = 0;
  const report = await evaluate(fixtures, {
    moderate: async () => { calls++; throw new Error("private provider response"); },
    classify: async () => { throw new Error("must not run"); },
  });
  assert.equal(calls, 1);
  assert.equal(report.complete, false);
  assert.equal(report.start.evaluated, 0);
  assert.deepEqual(report.error, { id: "S-01", stage: "SAFETY", code: "MODERATION_PROVIDER_FAILED" });
