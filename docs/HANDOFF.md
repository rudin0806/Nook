...
- 커밋 메시지만으로 의도를 단정하지 않는다. `ui: recover ... from failed deployment` 커밋들은 "배포 실패로 고아가 된 코드"가 아니라 **사용자가 되돌리기로 한 UI의 파일을 보존한 기록**이었다. 이 오해로 되돌린 디자인을 되살리는 병합을 했다가 취소했다.
- 브랜치 끝점만 검색하고 "어느 브랜치에도 없다"고 결론 내리지 않는다. 히스토리에 UI 커밋이 수십 개 있고 제거된 CSS가 과거 커밋에 남아 있다.
- 제품 의미가 달라지는 판단은 사용자에게 확인한다. 책등 유지 여부가 그 예다.
