...
## 문서·예시 화면 인수인계 — 2026-09-17 7차

### PRD는 v3이 기준이다

`docs/PRD.md`가 v3으로 교체됐다. 수치와 판정식은 PRD에 다시 쓰지 않는다 —
상한값·Confidence·프롬프트 계약은 `RULES.md`, 평가 계약은 `EVALSET.md`, 테이블·RPC는
`ERD.md`, 구현·검증 현황은 `STATUS.md`다. PRD에 같은 목록을 두면 두 곳이 어긋난다.

`서랍`은 폐기한 표현이다. 사용자 화면과 문서 모두 **생각 더미**를 쓴다.

### 예시 화면은 CSS만 깔려 있다

`data-sample` 스코프와 `.sample-band`·`.sample-chip`이 `redesign.css`에 있다. 화면(`/example`),
fixture, 홈 칩 연결은 아직 없다.

만들 때 지킬 것:

- **실제 컴포넌트에 고정 데이터만 주입한다.** 예시 전용 컴포넌트를 새로 만들면 제품과 어긋난다
- **홈 위 오버레이가 아니라 별도 라우트로 만든다.** 가짜 데이터가 실제 상태·DB·한도에
  흘러들 구조 자체를 없애기 위한 것이다
- **DB 쓰기와 모델 호출이 0이어야 한다**
- 예시용 클래스를 카드마다 붙이지 않는다. 스코프 안에서 토큰이 이미 덮인다
- 대화 1건만. 가짜 책장 20권은 만들지 않는다 — 그게 예전에 목업처럼 보였던 지점이다

### 예시 대화 전사는 이 환경에서 못 만든다

프록시가 `api.openai.com`과 `*.vercel.app`을 모두 403으로 막는다(확인함). 이 컨테이너에
`OPENAI_API_KEY`도 없다. 전사는 둘 중 하나로 만든다.

- 환경에 API credential을 등록한다(Pro·Max 플랜). 프록시가 세션 밖에서 헤더를 붙이므로
  키가 샌드박스에 들어오지 않고, 해당 호스트는 네트워크 레벨과 무관하게 열린다
- 또는 운영에서 직접 돌리고 텍스트를 붙인다

엔진(`src/engine/`)은 순수 함수이고 `JsonTransport`만 받는다. 키가 닿으면 DB·Next.js 없이
운영과 같은 프롬프트로 돌릴 수 있다.
...
- `card-stack.tsx`가 질문·휴지통의 스택이고, 카드 본문은 `retention-card.tsx`가 순서 편집 목록과 공유한다. 본문을 한쪽에만 고치지 말 것.
- 뒤 카드는 **위·옆으로** 삐져나온다(`--shift`/`--lift`). 아래로 내리면 다음 질문이 안 보여 레퍼런스와 달라진다.
- 상세 카드에는 스택을 그리지 않는다. 한 번 해봤고 읽히지 않아 되돌렸다. 상세는 `data-turn` 방향 넘김이다.
- 빈 상태 점선 카드의 오프셋은 `globals.css`의 스택 정의 옆에 있다. `redesign.css`에 다시 만들지 말 것(중복이 원인이었다).

### 아이콘 크기 — 건드리지 말 것 (2026-09-17)

- `NookIcon`의 svg는 `viewBox`만 있어서 `.nook-icon`의 width/height가 없으면 **0×0으로 렌더된다.** 실제로 그 상태로 오래 있었다.
- 크기는 `redesign.css`의 `.nook-icon`(26px)과 `.nook-icon[data-compact]`(22px)에 있다. 지우면 모든 아이콘이 사라진다.
- 하단 탭에서는 아이콘이 `color: inherit`으로 링크 상태를 따른다. 브랜드 톤을 되살리면 셋 다 활성처럼 보인다.
