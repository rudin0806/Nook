...
2026-09-16 문서 실사 이전 STATUS 원문을 보존한다. 현재 상태는 [STATUS](../STATUS.md)를 따른다. 서로 다른 시점의 미완료 문구와 완료 문구가 섞여 있으므로 이 파일로 현재 진척도를 판단하지 않는다.

## 2026-09-16 대화 노드 이동 복구·책장 순서 편집

기존 차분한 UI를 유지한 채 대화 노드 이동을 복구했다. 노드 선택 시 해당 질문이 시작된 메시지로 이동하고, 활성 노드를 다시 선택하면 현재 입력 위치로 돌아온다. 키보드 포커스와 모션 감소 설정을 지원하며 모바일 노드는 2열이다.

생각더미에는 책장 순서 편집을 추가했다. 데스크톱 드래그와 모바일·키보드용 위/아래 버튼을 제공하고, 전체 저장 목록을 서버에서 검증한 뒤 단일 RPC로 순서를 원자적으로 저장한다. 다른 탭에서 목록이 바뀌면 409로 저장을 거절한다. 운영 DB migration `saved_session_shelf_order` 적용 및 보안/성능 advisor 확인 완료. 타입·린트·프로덕션 빌드 통과. 미리보기와 운영 브라우저에서 기존 디자인 유지·홈·생각더미 조회를 확인했고, 운영 health 200 및 배포 후 런타임 오류 0건을 확인했다. 운영 배포 `dpl_387H51i2jBD3BP3iAQGMbp7j4bA3`이 두 운영 별칭에 연결됐다. 현재 운영 계정의 SAVED 세션은 1개라 다중 카드 드래그는 추가 데이터가 생길 때 사용자 화면에서 최종 확인한다. 단일 카드 배열로 RPC 성공·rollback을 확인했다.

---

## 2026-09-15 디자인 01

이전 무드보드를 폐기하고 Pretendard/SEED 기반 작업 공간으로 홈·대화·생각 더미·계정 화면을 재정비했다. [디자인 명세와 시안](design/README.md). API·보관 정책 변경 및 실제 모델 호출 없음. 로컬 브라우저 접근 제한으로 시각 검수는 미완료이며 운영 배포는 하지 않았다.
