...
이 판정은 상태 추정이 아니다. 지쳤는지 방어하는지는 묻지 않고 알 수도 없다. 세는 것은 글자 수뿐이며, 자기 기준과 비교하므로 원래 짧게 답하는 사용자에게는 걸리지 않는다. 계산은 `src/engine/stall.ts`가 하고 Judge에는 `stalled` boolean만 들어간다 — hedge_speaker와 같은 구조다. 임계값(`STALL_RUN` 2회, `STALL_RATIO` 0.5, `STALL_ABSOLUTE` 20자, `STALL_MIN_HISTORY` 2)은 코드 상수이며, 실제 세션이 쌓이면 조정한다.
