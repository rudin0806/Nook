...
Shift가 아니지만 나중에 Nook에서 독립적으로 생각해볼 질문이면 Branch로 남긴다.
...
Pile 항목과 같은 방향의 **자발적 발화가 HIGH 조건을 채우면** Shift로 승격 가능하다.

```json
{ "promote_pile_item": "P1" }
```

`promote_pile_item`은 입력 Pile에 실제로 존재하는 id만 허용한다. HIGH SHIFT가 아닐 때는 `null`이어야 한다.
...
- `SAFETY_STOPPED`와 동일시하지 않는다. **정확한 DB lifecycle status는 ERD에서 별도 확정한다.**
- 자동으로 `지나온 생각`에 저장하지 않는다.
...
- 남기기 → 확정 Main Path는 `지나온 생각`, Branch/Pile은 `남겨둔 질문`
- 남기지 않고 나가기 → 이번 세션과 이번 세션에서 생긴 Pile Item을 보관하지 않음
- 기존 저장 기록은 영향 없음
- 서랍/더미를 별도 목적지처럼 선택하게 하지 않음
- Safety STOP에서는 이 보관 선택을 묻지 않음
