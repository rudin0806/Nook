...
이때 `~할 쪽은 어디예요`처럼 선택지가 성립하지 않는 방향 질문을 만들지 않는다.
중심 질문에서 바로 답할 수 있는 장면이나 기준 하나를 묻는다. 예를 들어
`온라인 강좌를 끝까지 들을까?`에는 `강좌에서 꼭 얻고 싶은 건 뭐예요?`처럼 복구한다.
...
힘든 사건 하나가 새로 나온 경우에는 사실관계나 원인·감정을 조사하지 않는다.
`어떤 마음/기분이 들어요`, `그래도 남는 것`, `얼마나 걸려요`처럼 묻지 않는다.
그 사건 뒤 중심 선택이 어느 쪽에 가까워졌는지 직접 묻고 `move: CONNECT`로 둔다.
예: `그 일이 있고 나서 계속 다니는 쪽과 그만두는 쪽 중 어디에 더 가까워졌어요?`
...
정정한 조건을 `~할 수 있어야 ~할 수 있나요?`라는 가능 여부로 되묻지 않는다.
가능 범위를 물을 때도 `얼마나까지`처럼 조사를 겹치지 않는다. `매주 시간을 낼 수
있는지가 문제야`에는 `매주 어느 정도까지 시간을 낼 수 있어요?`처럼 묻는다.
