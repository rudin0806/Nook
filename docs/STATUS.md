...
- **현재 작업 브랜치: `claude/supabase-github-public-setup-u6t79x` / `80a2a90`.** 위 보존 브랜치 `e1bf2e1`에서 갈라져 나왔고 아래 4개 커밋이 얹혀 있다. 기능 코드의 최신은 이 브랜치다.
...
- **운영 앱은 재배포하지 않았다.** 위 4개 커밋은 GitHub에만 있고 Vercel 운영은 여전히 `dpl_387H51i2jBD3BP3iAQGMbp7j4bA3`이다. 즉 벤또 홈·라이트 모드·책장 이동 UI는 운영에 없다. 운영 DB에는 아래 migration이 적용돼 있으므로 **DB는 앞서 있고 앱은 뒤에 있는 상태**다. 옛 `reorder_saved_sessions`를 남겨둔 이유가 이것이며, 재배포 전에 제거하면 운영 순서 저장이 깨진다.
...
사용자가 지적했고 아직 안 한 것들이다. **책등 표현과 책상 컨셉은 유지한다** — `docs/design/README.md`의 "세로 책등 표현을 제거한다"는 문구는 뒤집혔다.

- **메뉴를 상단탭으로.** 현재 `globals.css`의 `.app-dock`이 좌측 고정 사이드바다. vivid `redesign.css:67`에 상단탭 스타일이 있다(`flex-direction: row`, 아이콘 숨김, 선택 탭 밑줄, `.desk-main` 좌측 오프셋 제거 필요).
- **이어갈 대화는 카드 양식.** 빈 자리는 겹쳐 기울어진 점선 카드 3장이어야 한다. vivid `redesign.css:646-649`에 있다.
...
- 입력창 테두리가 시안만큼 보이지 않는다. SEED `TextField.Root`가 자기 스타일을 갖고 있다.
