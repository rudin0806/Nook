...
- 탈퇴가 즉시 삭제에서 **30일 유예**로 바뀌었다(사용자 결정). 신청하면 `account_deletions`에 표시만 남고 바로 로그아웃되며, 30일 안에 다시 로그인하면 본인이 취소할 수 있다. 기한이 지나면 `purge_expired_accounts()`가 계정을 파기하고, 기존 시간당 cron(매시 17분)이 세션 정리와 함께 이 함수를 호출한다.
- 표시는 클라이언트가 못 쓴다. `authenticated`에 SELECT만 주고, 파기 함수는 `service_role`에만 실행 권한을 준다. 즉시 삭제용 `delete_own_account()`는 제거했다.
- SQL 회귀: 유예 창이 한 달인지, 재신청이 기한을 늘리지 않는지, 기한 전 파기가 0건인지, 취소가 되고 두 번은 안 되는지, 기한 후 파기가 본인 것만 지우는지, `authenticated`가 파기 함수를 못 돌리는지 확인한다.
