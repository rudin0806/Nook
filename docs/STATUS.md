...
## 스티키 헤더·생각 더미 빈 상태·타이포 위계 — 2026-09-18 13차

### 스크롤할 때 제목이 탭 행을 통과한 이유

`.app-dock`이 sticky이긴 했는데 배경이 `--nook-sticky`(알파 0.82) + `backdrop-filter`
blur였고 `margin: 0 40px`로 좌우가 들려 있었다. 반투명이라 글자가 비쳤고, 양옆 40px로는
콘텐츠가 그냥 지나갔다. 전면(`margin: 0; padding: 0 40px`) + 불투명
`--nook-paper` + blur 제거. 측정: 배경 `rgb(244,245,247)`, blur `none`,
좌 0 / 우 = 뷰포트 폭.

### 타이포 위계

`생각 더미`는 브라우저 기본 h1 크기에 크기 지정이 없는 부제를 붙여 쓰고 있었다. 그래서
아무 순위도 안 보였다. 홈의 `.desk-heading` 묶음을 그대로 쓰게 하고 한 스케일로 통일했다.
측정: kicker 12 / h1 34 / 부제 15px, 두 화면 동일.

**부 타이틀에 온점을 찍지 않는다.** `지나온 질문을 다시 펼쳐보는 자리`,
`다시 만나 반가워요`, `내 생각을, 내 자리에`, `남겨둔 질문들이 여기 있어요`,
`보관한 생각을 다시 펼쳐보는 계정`에서 제거했다. 본문 문장의 온점은 유지한다.

### 생각 더미 빈 상태

비로그인 상태를 `error`로 그리고 있었다 — 평범한 카드에 **녹색 링크**와 회색 버튼, 화면
어디와도 안 맞았다. 홈 패널과 같은 언어로 바꿨다: 점선 프레임 + 같은 책 일러스트 +
포인트 컬러 채움 버튼 하나. 문구도 `계정을 연결해 주세요`가 프레임 제목과 겹쳐서
`로그인하면 남긴 이야기와 질문을 여기서 볼 수 있어요`로 바꿨다.

컬렉션 전환은 세그먼트 컨트롤로 보이게 했다(알약 + 선택된 것만 포인트 컬러). 검정
배경이 이 화면의 유일한 검정 면이었다. `목록 새로고침`은 왼쪽 여백에 굵은 글씨로 놓여
있었는데 눌릴 것처럼 보이지 않았다 — 작은 칩으로.

### 홈 세부

- `계정 연결하기`가 프레임을 가득 채운다. 양 끝이 패널 콘텐츠 경계에 붙어서 조명 토글과
  같은 열에 놓인다(토글 우측 기준 21px 안쪽 — 프레임 자체의 여백만큼)
- `이어갈 대화` 패널 하단 여백 확보. 카드 더미가 아래 경계에 붙어 있었다

측정: 가로 오버플로 없음(홈·생각 더미 모두 1280=1280).
...
`.lamp-panel[data-lit="true"] .nook-icon { color: var(--nook-open) }`가 타일 규칙보다
구체적이어서, **주황 타일 위에 주황 글리프**를 그리고 있었다. 타일이 톤을 들고 있으므로
이 규칙은 할 일이 없어졌다 — 지웠다. 측정: 글리프 `rgb(255,255,255)` / 배경
`rgb(255,154,87)`.

### 그 외

- **타일**: 상자 38px 유지, 패딩 8→4px로 글리프를 키웠다. 기울기를 톤별로 달리했다
  (`-5deg / 6deg / -7deg`) — 같은 각도 셋은 반복된 실수처럼 읽힌다
- **제목 간격**: 탭 행 바로 밑에 붙어 있었다. 49px 확보
- **책장 일러스트**: 막대 그래프였다(같은 너비, 평평한 위, 선 위에 나란히). 너비를
  다르게, 하나는 기울이고, 책등 밴드를 `::after`로 넣어 책으로 읽히게 했다. 프레임
  높이도 줄였다
- **이어갈 대화 빈 카드**: 띠처럼 납작해져 있었다. `min-height: 176px`로 키웠다
- **좌우 높이**: `align-items: stretch`로 맞췄다. 측정 741 = 741, 차이 0

입력란을 컬럼 높이에 맞춰 늘리려 `height: 100%`, flex stretch, `position: absolute`

- `inset: 0`를 차례로 시도했지만 **SEED가 필드 내부 박스를 소유해서 전부 텍스트영역까지
  닿지 않았다**(`position: absolute`·`top/bottom: 0`가 계산값에 반영됐는데도 높이가
  107px로 남았다). 파생하는 대신 `min-height: 420px`로 직접 요구했다. 남은 여백은 약
  90px.

### 탭 제목·설명

`Nook — 생각을 잠시 놓아두는 곳` / `답을 주는 대신, 내가 어떤 질문을 지나왔는지.` 였다.
PRD v3의 소개 문구로 맞췄다: **`Nook — 복잡한 생각, 말하면서 정리해요`**.

## 홈 화면 품질·넓게 쓰기 모션 — 2026-09-18 10차

### 왜 허전해 보였는가

라이트 모드의 `--nook-card-line`이 `transparent`였다. 흰 패널이 거의 흰 페이지
위에 아무 경계 없이 놓여 있었고 그림자도 `none`이었다. 실선 헤어라인과 낮은
그림자 한 겹을 넣어 벤또가 별개의 면으로 읽히게 했다.

### 고친 것

- **아이콘**: 26px 선 아이콘이 제목 옆에서 부스러기처럼 읽혔다. `data-tile`로
  38px 둥근 사각형에 톤 배경 + 흰 글리프. 타일은 `-5deg` 기울이고 글리프는
  `+5deg` 되돌려서 스티커처럼 보이되 아이콘은 똑바로 선다
- **`전체 보기` 밀림**: 라벨에 `↗`가 붙어 있었는데 이 글자는 베이스라인 위에
  떠서 라벨을 밀어낸다. 쉐브론을 `::after`로 분리했다
- **`계정 연결하기`**: `--nook-primary-weak` 배경에 primary 글자 — 동작으로
  안 읽혔다. 같은 클래스에 규칙이 두 개 있었고 더 구체적인 옛 규칙이 이기고
  있었다. 옛 규칙을 지우고 포인트 컬러 채움으로 통일했다
- **`시작하기`**: 포인트 컬러 채움 + 종이색 라벨. 화살표 `↗` → 쉐브론 `›`
- **`넓게 쓰기`**: 링크가 아니라 모드 전환이므로 작은 컨트롤 형태로. 화살표 제거
- **빈 상태**: 책장만 점선 프레임을 갖는다. 이어갈 대화는 이미 점선 카드를
  그리므로 프레임을 씌우면 같은 신호가 두 겹이 된다. 프레임에 `--nook-fill`
  배경을 넣었더니 `--nook-raised`로 그린 책 막대 세 개 중 두 개가 같은 값이라
  사라졌다 — 배경을 뺐다
- **하단 문구 삭제**: `결론이 나지 않아도 괜찮아요`가 벤또 아래에 다른 요소와
  같은 무게로 놓여 있었고 인트로와 같은 말이었다

### 넓게 쓰기 모션

`home-bento`를 4-area 맵에서 2열로 바꾸고 오른쪽 패널 셋을 `<aside>` 하나로
묶었다. 격자가 통째로 치울 수 있는 하나가 된다.

`grid-template-columns`를 전환한다. 측정: `803px 382px` → **`1186px 0px`** →
바깥 클릭 후 `803px 382px`. aside는 같은 시간 동안 `opacity: 0` +
`translateX(28px)`로 빠진다.

상태는 `HomeDesk`가 갖는다. 바뀌는 게 패널이 아니라 페이지 격자이기 때문이다.
`ThoughtInput`은 controlled/uncontrolled 양쪽을 받는다 — 홈 밖에서는 패널이
이미 한 열 전부라 스스로 관리한다.

여는 조건은 `넓게 쓰기` 버튼과 **입력란 포커스**. 닫는 조건은 패널 바깥
pointerdown과 Escape. `dialog`와 `iframe` 안의 클릭은 바깥으로 치지 않는다 —
캡차 프레임이 페이지 위에 겹쳐 뜨기 때문이다.

`aria-hidden` 대신 `inert`를 썼다. aria-hidden은 포커스 가능한 링크를 탭
순서에 남긴 채 스크린리더에서만 감춘다.

모바일(900px 이하)은 원래 한 열이라 넓힐 것이 없다. `data-expanded`가 aside를
숨기지 않도록 되돌린다.

측정: 가로 오버플로 없음(1280=1280, 390=390), 타일 4개 38×38
`rgb(108,107,232)`, CTA 배경 `rgb(108,107,232)`, footnote 0개.
...
운영 배포에 `ssoProtection: { enabled: true, deploymentType: "all_except_custom_domains" }`가
걸려 있었다. `nook-nine-eta.vercel.app`은 Vercel이 부여한 도메인이라 여기 포함되므로,
**팀 접근 권한이 없는 사람은 로그인 벽을 만난다.** 대회 제출 링크로는 치명적이다.
사용자 승인 후 껐고, API 응답으로 `enabled: false`를 확인했다.

앱 자체의 `robots: noindex, nofollow`는 그대로다. 링크를 아는 사람만 열 수 있고 검색에는
잡히지 않는 상태다. 로그아웃 상태 접속 확인은 이 환경에서 못 한다 — 프록시가
`vercel.app`을 막는다. 시크릿 창 확인은 사용자 몫.

### 첫 질문 확인 화면

운영에서 직접 써보고 나온 결함 네 개를 고쳤다.

- **제안 질문 입력창이 240px였다.** 한 문장을 5,000자 사고 입력면에 넣고 있었다.
  `question-field`를 추가해 84px로 내렸다 (측정: 사고 단계 240px / 제안 단계 84px)
- **주 동작이 `neutralWeak`였다.** `시작하기`와 `이 질문으로 확정하기`가 `나가기`·
  `다른 생각 적기`와 같은 무게였다. `neutralSolid`로 올렸다. SEED의 `brandSolid`는
  Nook 색이 아니라서 쓰지 않았다 (측정: `variant_neutralSolid`)
- **같은 말을 네 번 했다.** 제목 `이 질문으로 시작할까요?` + 근거 문장 + `내 뜻과 맞게
수정할 수 있어요. 아직 확정된 기록은 아니에요.` + 버튼 `확정하기`. 확인 횟수는
  스펙대로 한 번이지만 문구가 겹쳐서 여러 번 묻는 것처럼 읽혔다. 하단 문장을
  `고치고 싶으면 그대로 고쳐도 돼요.` 한 줄로 줄였다
- **이어갈 대화 빈 상태의 점선 카드가 패널 밖으로 나갔다.** 내가 7차에서 넣은 결함이다.
  `.empty-cards`가 `inset: 0`인데 자식 카드를 최대 +9px/+22px 이동시키고 회전까지 해서
  바깥으로 삐져나갔다. `inset: 34px 24px 30px`으로 안쪽에 여유를 만들었다.
  측정: 3장 모두 네 변 전부 패널 안쪽, 최소 여유 19.5px (모바일 하단)

### 캡차가 첫 화면에 있었다

비로그인 방문자가 **아무것도 적기 전에** `계정 없이 시작하기 위한 사용자 확인` 캡차를
봤다. "적기 전에 정리하지 않아도 된다"는 제품의 첫 약속과 정면으로 어긋난다.

첫 글자를 입력한 뒤에 마운트하도록 조건을 걸었다. 글을 쓰는 동안 로드·해결될 시간을
확보하면서 첫 화면은 비워둔다. **서버 계약은 그대로다** — Supabase가 토큰을 검증하고,
토큰 없는 제출은 여전히 실패한다.

이 환경에서는 `NEXT_PUBLIC_HCAPTCHA_SITE_KEY`가 없어서 위젯 자체를 띄울 수 없다.
운영 배포 후 시크릿 창에서 **빈 입력창에 캡차 없음 → 한 글자 입력 시 나타남**을
확인해야 한다. 근본 해결은 hCaptcha invisible 모드 + 제출 시 `execute()`이며 아직 안 했다.
